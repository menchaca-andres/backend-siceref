...
export const MascotaModel = {
    ...
    create: async (data: CreateMascotaDto) => {
      ...
      RETURNING *
    `, [
            data.id_raza, data.img_mascot, data.nom_mascot, data.edad_mascot,
            data.fenac_mascot, data.descrip_mascot, data.gen_mascot, data.esterilizado
        ])
        return result.rows[0]
    },
    ...
    update: async (id: number, data: UpdateMascotaDto) => {
      ...
      UPDATE MASCOTAS SET
        ...
        descrip_mascot = $4, esterilizado = $5
      WHERE id_mascot = $6
      RETURNING *
    `, [
            data.img_mascot, data.nom_mascot, data.edad_mascot,
            data.descrip_mascot, data.esterilizado, id
        ])
        return result.rows[0]
    },
